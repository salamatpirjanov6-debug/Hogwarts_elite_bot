...
BAD_WORDS = ["bla", "yban", "iflos", "axmoq"]
...
# --- 1. KITOOBLAR BAZASI (TO'LIQ TIKLANDI) ---
BOOKS_UZ = [
...
# --- 2. KINOLAR BAZASI (TO'LIQ TIKLANDI) ---
MOVIES_UZ = [
...
async def check_sub(user_id):
    try:
        ...
        return m_ch.status in ["creator", "administrator", "member"], m_gr.status in ["creator", "administrator", "member"]
...
async def mute_cmd(message: types.Message):
    ...
    if message.reply_to_message:
        duration = int(message.get_args()) if message.get_args().isdigit() else 15
